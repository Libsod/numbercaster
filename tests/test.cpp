...
TEST(InputTest, Valid) {
  std::istringstream input("52.0"); // Simulate input of "52.0"
  ...
  EXPECT_EQ(result.value(), 52.0); // Assert that the returned value is 52.0
}

/* Test case to handle invalid input for getInput() function */
TEST(InputTest, Invalid) {
...
TEST(BoundsTest, BelowLowerBound) {
...
TEST(BoundsTest, AboveUpperBound) {
...
TEST(NumberCastTest, CastingToI32) {
  f64 value = 52.0; // Define a floating-point number
  EXPECT_EQ(static_cast<i32>(value),
            52); // Assert that casting to i32 yields 52
}

/* Test case to check getMax() function with positive integers */
TEST(GetMaxTest, Positive) {
